Sys::OsRelease::Lite - read operating system details from standard /etc/os-release file
...
Current attributes recognized by Sys::OsRelease and Sys::OsRelease::Lite are:
    ...
    PORTABLE\_PREFIXES PORTABLE\_SCOPE
...
[Sys::OsRelease](https://metacpan.org/pod/Sys%3A%3AOsRelease) 0.4.0 had to follow an increase in the minimum Perl version from 5.10 to 5.22.
For systems with Perl older than 5.22, see below about [Sys::OsRelease::Lite](https://metacpan.org/pod/Sys%3A%3AOsRelease%3A%3ALite)
...
_Sys::OsRelease_ uses a singleton model. So there is only one instance.
...
- import\_singleton()
    ...
    The singleton-management methods _init_, _new_, _instance_, _defined\_instance_ and _clear\_instance_
    can be imported by another class by using the import\_singleton() method.
    That was done for [Sys::OsPackage](https://metacpan.org/pod/Sys%3A%3AOsPackage), to allow it to avoid copying those methods.
    But other classes with a similar need to minimize module dependencies which already
    use _Sys::OsRelease_ can do this too.
...
For convenience, _Sys::OsRelease_ generates read-only accessor methods for each of the standard 
attribute names, converted to lower case. For example, from the list above they are _name()_, _id()_,
_id\_like()_, etc. The auto-generated methods do not require any parameters, and ignore any if provided.
...
they could conflict with existing functions in the _Sys::OsRelease_ namespace.  Use the _found\_attrs()_,
_has\_attr()_ and _get()_ methods to detect and access non-standard attributes.
...
- [Sys::OsPackage](https://metacpan.org/pod/Sys%3A%3AOsPackage)
...
- [System::Info](https://metacpan.org/pod/System%3A%3AInfo)

    system information collected from multiple sources including system architecture, hardware, OS release data

- [Sys::OsRelease::Lite](https://metacpan.org/pod/Sys%3A%3AOsRelease%3A%3ALite)
    ...
    It is packaged with [ExtUtils::MakeMaker](https://metacpan.org/pod/ExtUtils%3A%3AMakeMaker) to maintain availability back to Perl 5.10.
